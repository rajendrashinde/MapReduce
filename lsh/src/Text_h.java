...
public class Text_h {
	...
	public static class Map extends MapReduceBase implements Mapper<LongWritable, Text, Text, Text> {
		...
		public void map(LongWritable key, Text value, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
			...
			if ( rnd.nextFloat() < 0.8 ) {	
				...
				h_bucket.set(LSH_functions.hbucket(point, k, W));
				...
				for (int i = 0; i < L; i++){
				    ...
				    junk.set(LSH_functions.hbucket(query.plus(perturbation), k, W));
				    f.add(junk);
				}
	...
	public static void main(String[] args) throws Exception {
        JobConf conf = new JobConf (lsh.Text_h.class);
        conf.setJobName("H_job");
        conf.setOutputKeyClass(Text.class);
        conf.setOutputValueClass(Text.class);
        conf.setMapperClass(lsh.Text_h.Map.class);
        conf.setReducerClass(lsh.Text_h.Reduce.class);
